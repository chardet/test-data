...
def load_existing_notes(catalog_path):
    ...
    with open(catalog_path, encoding="utf-8") as f:
        for line in f:
            ...
            # 5-column: | `file` | source | provenance | size | notes |
            # Falls back to the pre-provenance 4-column layout so notes
            # written before that column existed are still carried over.
            m = re.match(
                r'^\| `([^`]+)` \| [^|]+ \| [^|]+ \| [\d,]+ \| (.*?) \|$', line
...
def provenance_of(dirname, filename):
    """Provenance of one file: wild, transcoded, or suite.

    Derived from the commit that introduced the file (see scripts/
    provenance.py).  Falls back to an empty cell if git is unavailable, so
    the catalog can still be regenerated outside a checkout.
    """
    global _PROVENANCE  # noqa: PLW0603
    if _PROVENANCE is None:
        try:
            from provenance import classify_all  # noqa: PLC0415

            _PROVENANCE = classify_all()
        except Exception as exc:  # noqa: BLE001
            print(f"warning: provenance unavailable ({exc}); leaving column blank")
            _PROVENANCE = {}
    return _PROVENANCE.get(f"{dirname}/{filename}", "")


def provenance_summary_lines(all_dirs):
    """Markdown for the per-encoding wild-vs-transcoded coverage section."""
    provenance_of("", "")  # prime the cache
    if not _PROVENANCE:
        return []

    per_encoding = {}
    for dirname, files in all_dirs.items():
        enc = get_encoding_from_dirname(dirname) or dirname
        stats = per_encoding.setdefault(enc, {"wild": 0, "transcoded": 0,
                                              "suite": 0, "total": 0})
        for fname, _size in files:
            stats["total"] += 1
            verdict = _PROVENANCE.get(f"{dirname}/{fname}")
            if verdict in stats:
                stats[verdict] += 1

    totals = {"wild": 0, "transcoded": 0, "suite": 0, "total": 0}
    for stats in per_encoding.values():
        for key in totals:
    ...
    lines = [
        ...
        "",
        f"Repo totals: **{totals['wild']:,} wild**, "
        f"{totals['transcoded']:,} transcoded, {totals['suite']:,} suite "
        f"(of {totals['total']:,} files).",
        "",
    ...
    for enc in sorted(per_encoding, key=lambda e: (-per_encoding[e]["wild"], e)):
        stats = per_encoding[enc]
        lines.append(
            f"| `{enc}` | {stats['total']:,} | {stats['wild']:,} | "
            f"{stats['transcoded']:,} | {stats['suite']:,} |"
        )
    lines.append("")
    return lines
...
def main():
    # Allow "from provenance import ..." when run from any directory.
    ...
    # Provenance section (wild vs transcoded, per encoding)
    lines.extend(provenance_summary_lines(all_dirs))
    ...
    if ascii_dirs:
        ...
        for adir in ascii_dirs:
            ...
            lines.append("")
            lines.append("| File | Source | Provenance | Size | Notes |")
            lines.append("|------|--------|------------|-----:|-------|")
            for fname, size in all_dirs[adir]:
                source = detect_source(fname)
                notes = get_notes(adir, fname, size, existing_notes)
                prov = provenance_of(adir, fname)
                lines.append(
                    f"| `{fname}` | {source} | {prov} | {size:,} | {notes} |"
                )
            lines.append("")
    ...
    for group_name, _ in ENCODING_GROUPS:
        ...
        for dirname in dirs:
            ...
            for fname, size in files:
                ...
                prov = provenance_of(dirname, fname)
                lines.append(
                    f"| `{fname}` | {source} | {prov} | {size:,} | {notes} |"
                )
            lines.append("")
